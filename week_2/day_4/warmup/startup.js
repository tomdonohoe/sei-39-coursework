...
function getRandomNumberInRange(maxNumRange) {
    return Math.floor(Math.random() * maxNumRange);
};

function generateStartupIdea(arrayOne, arrayTwo) {
    var arrayOneIndex = getRandomNumberInRange(arrayOne.length);
    var arrayTwoIndex = getRandomNumberInRange(arrayTwo.length);

    return `${arrayOne[arrayOneIndex]} for ${arrayTwo[arrayTwoIndex]}`;
};

for (var i = 0; i < 6; i++) {
    console.log(generateStartupIdea(thatArray, thisArray));
};